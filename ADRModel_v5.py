...
#         symptomToken = self.tokenizer.encode(symptom)[1]
        ...
        for i in range(len(self.df)):
            ...
            if symptomToken in self.tokenizer.encode(self.df.iloc[i]['message'].lower()):
                ...
                if not np.any(hidden_states[tokenIndex,:]):
                    continue
    ...
    def getSimilarWords(self, symptom, meanEmb, similarityThreshold = 0.3):
        ...
        #print('self.combinedOutputFolder', self.combinedOutputFolder)
    ...
    def getOutput(self, out):
        ...
        for i in range(len(output)):
            if len(output[i][0]) <= 2 or output[i][0] in stop_words:
                print(output[i][0])
                continue
    ...
    def exploreNode(self, word, depth, maxDepth = 3, topk = 5):
        ...
        if self.graph[word].vector is None:
            ...
            meanEmb = np.mean(meanEmb,0) #meanEmb will have context embedding of keyWord
            ...
            meanEmb = meanEmb.reshape(1, -1)
            self.masterEmb = self.masterEmb.reshape(1, -1)

            #print(meanEmb[0])
            #print(meanEmb.shape)
            #print(self.masterEmb.shape)
